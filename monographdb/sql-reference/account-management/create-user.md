---
title: CREATE USER
summary: An overview of the usage of CREATE USER for the MonographDB database.
---

# CREATE USER
Create a MonographDB account
...
* `authentication_option`: Specifies the authentication method of the account, which supports the following three methods in MonographDB
...
* `lock_option`: MonographDB supports account locking, allowing privileged administrators to lock or unlock user accounts
...
```
>**Note**
>The premise of using the `ed25519` plugin is that MonographDB has loaded the corresponding plugin. If it is not loaded, you need to execute the following SQL statement to install it.
>```sql
...
Create an anonymous user with the host name `localhost` using the following statement
  ...
  ```
  Connect to MonographDB anonymously using the following statement
  ```bash
  mysql -u '' -h localhost
  ```
