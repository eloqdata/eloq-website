...
## Make DynamoDB easy to use: Twitter Example
...
Let's pick two common access patterns in Twitter app:

1. given a tweet, return the top 5 latest replies with tweet content, replier name, reply time. This access pattern is used to display the latest replies when user click a tweet.
2. given a tweet, return the top 5 replies having most stars with tweet content, replier name, reply time and star count. This access pattern is used to display the top favorate replies when user click a tweet.
...
Pattern #1.
```
SELECT username, rep_content, rep_time FROM tweet_reply, user WHERE tweetid=100 and rep_user_id=userid ORDER BY rep_time DESC LIMIT 10;
```
...
Customer's business may change rapidly, the access pattern will change accordingly. For example, customer needs to get the top 3 nations and their count where the tweet's repliers belong to given a tweer id. But one user could reply a tweet multiple times, which require to remove the duplicated repliers before calculate the top 3 nations. This needs application side code change by add logic to remove the duplicated repliers when you use DynamoDB native SDK. This is costly and error-prone.

With MonoSQL, access pattern change can be handled by tunning of SQL script, which is easier. For example, removing the duplicated repliers can be handled by `distinct` keyword in SQL.
...
The TPS increases to 232533. The experiment indicates the linear scalability of MonoSQL. When you hit the write bottleneck of MySQL, switch to MonoSQL seamlessly.
