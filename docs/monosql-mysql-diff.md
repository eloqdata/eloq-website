...
Note that 1. the value of a column with a secondary index cannot be empty string or empty binary, but can be NULL. 
2. the length of secondary index name should be in range of 3 to 255.
...
In general, the data written to DynamoDB is Human-readable, that is, the data displayed by MySQL is basically the same as that displayed by DynamoDB. Exceptions: 1. the primary key has N columns (N>=3), the 2nd to N columns will be packed into binary form and stored in DynamoDB.
2. data type like `timestamp`, `year`, `time` are stored in number format, which is human-readable in MySQL but not in DynamoDB. It is recommended to use `datetime` to represent time type in MonoSQL.
