...
# 附录：使用 Data Substrate 组装你的数据库

在当今数据驱动的世界里，企业在高效管理与利用数据方面面临着日益严峻的挑战。传统数据库系统往往难以及时适应快速变化的业务需求，导致系统架构复杂、扩展刚性强、运维负担沉重。Data Substrate 正是在这样的背景下诞生的一层革命性抽象，它让你能够根据自己的具体需求，组装出高度定制、动态可扩展的数据库系统。

## 什么是 Data Substrate？

可以把 Data Substrate 想象成数据管理方案中“隐形的脊梁骨”。它是一个统一的平台，将各种数据场景中共性的关键能力抽象并封装起来。从保证一致性与持久性，到处理并发与容错，Data Substrate 为你打好了最基础、最核心的地基，让你可以把精力集中在业务真正需要的“定制数据库”之上。

此外，Data Substrate 的模块化设计以及纵向扩展能力，使你可以按需定制各个层级。这意味着你可以根据工作负载压力，分别扩展计算引擎、Data Substrate 本身以及云存储层，从而获得最佳的性价比。它还通过利用低成本的存储方案来托管冷数据，显著提升整体资源利用率。借助丰富多样的计算引擎，你可以打造高度契合应用需求的数据库：无论是面向在线事务处理的高性能 SQL 引擎，适合灵活内容管理的 KV 引擎，还是支持任意用户自定义函数的 Wasm 引擎，都可以在同一底座上进行组合与扩展。
...
- **计算引擎（Compute Engines）**：位于架构的最上层，可按需选择 SQL、KV、文档、图等多种计算引擎。这些引擎与 Data Substrate 无缝集成，为不同类型的计算与分析场景提供弹性支持。
- **Data Substrate**：处于中间层，是整个架构的“骨干”，主要提供以下能力：
  - **缓存（Caching）**：将热点数据保存在内存中，加速访问。
  - **并发控制（Concurrency Control）**：保证事务满足 ACID 特性，并支持多写（multi-writer）架构。
  - **数据持久化（Data Persistence）**：在系统故障时仍能保证数据不丢失。
  - **一致性（Consistency）**：在多组件、多操作间保持数据的逻辑一致。
  - **容错（Fault Tolerance）**：提高系统的鲁棒性，在错误发生时能快速恢复、避免数据丢失。
- **云存储（Cloud Storage）**：Data Substrate 可以集成多种云存储方案，如 AWS DynamoDB 与 Google Bigtable，主要承担两类职责：
  - **冷数据存储**：将访问频率较低的数据以更低成本存放在云存储中，从而减少计算资源消耗。
  - **缓存未命中处理（Cache Miss Handling）**：在缓存中未命中时，从云存储中拉取数据，保证整体数据完整可访问。
  - **打破厂商锁定（Vendor Lock-in）**：借助多云与混合云架构，你可以在不同云之间自由调度数据，获得真正的云中立性。

## 为什么选择 Data Substrate？

现代企业需要的是能够灵活应对各种工作负载的数据系统，而不仅仅是一套固定形态的数据库。Data Substrate 帮助你摆脱传统数据库的诸多限制，并且直面以下常见痛点：

- **冗长复杂的数据处理链路**：Data Substrate 可以显著简化数据流，减少重复处理环节，让整体数据架构更为清晰。
- **重复造轮子的通用功能开发**：你无需再为缓存、一致性、持久化等基础能力反复手写代码，Data Substrate 已经提供了坚实可靠的底座。
- **资源利用率低下**：Data Substrate 能够对不同组件进行精细化弹性扩展，使资源的分配更贴近实际负载，避免长期闲置或过度超配。
- **数据同步不一致**：借助事务型缓存机制，Data Substrate 可以打破数据孤岛，帮助你在系统内部维持端到端的一致性。
- **多云与混合云挑战**：Data Substrate 让你可以在多云环境下灵活编排数据流向，以最契合业务策略的方式在不同云之间迁移或复制数据。

## Data Substrate 的关键特性

Data Substrate 通过极致的弹性与适配能力脱颖而出。它可以根据负载变化自动调整资源分配，在各种场景下都能保持良好性能。下面是几个关键亮点：

- **弹性扩展（Elastic Scaling）**：可以轻松应对各类工作负载。在读多写少的场景下，Data Substrate 能够扩展其内存与节点数量，实现分布式缓存与极致的读取时延。
- **并行写优化（Parallel Write Optimization）**：不再出现写入瓶颈。Data Substrate 采用专利的一阶段提交协议（one-phase commit）与并行日志写入能力（parallel logging），即便在写入压力极高的场景下，也能在保证持久性的前提下保持高可用。
- **高性价比的扩展能力（Cost-Effective Scalability）**：面对超大规模数据集，Data Substrate 可以在不显著增加计算资源的情况下，将数据横向扩展到云存储，把真正冷的数据交给低成本存储托管，大幅降低整体成本。
