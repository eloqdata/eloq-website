...
人工智能正在迅速成为有史以来最大的计算基础设施消耗者之一。训练与推理系统如今依赖规模在几年前尚不存在的巨型 GPU 集群。在关注点大多集中在算力的同时，这些系统也对高性能内存产生了前所未有的需求，给全球 DRAM 供应链带来新的压力。

其结果便是内存经济学的剧烈变化。随着 AI 基础设施的扩张，全球 DRAM 产能中很大一部分被分配给以 GPU 为中心的系统，使更广泛技术市场的供给趋于紧张。2025 年市场的 DRAM 平均售价涨幅超过 400%，分析人士预计 2026 年压力仍将持续。在许多情况下，即便愿意承受更高成本，企业仍难以获得额外的内存容量。

因此，工程团队开始重新审视严重依赖大规模内存数据集的架构。围绕充裕且廉价的 DRAM 设计的系统，正面临新的经济约束。在内存充足的时代，这种设计思路很少受到质疑；如今，这些假设正在发生改变。


## **以 DRAM 为先的架构的局限**
许多现代应用通过将活跃数据集完全置于内存中来实现极低延迟。Redis 等技术通过支持对存储在 DRAM 中的数据结构进行快速访问，使这一模式得以普及。

由于 Redis 被设计为内存型系统，实际可支撑的数据集规模与可用于存放活跃工作集的内存容量紧密绑定。随着数据集增长，必须增配 DRAM 才能维持性能。在内存既越来越贵又越来越难获取的环境下，这种依赖开始对系统规模施加新的经济上限。

这些变化引出一个根本问题：高性能数据系统如何在不必将全部数据集常驻内存的前提下，仍保持低延迟？

## **EloqKV 简介**

EloqKV 正是为应对这一挑战而设计。它不假定高性能数据系统必须将全部活跃数据集放在 DRAM 中，而是采用以 SSD 为先的架构，充分利用现代 NVMe 存储的性能。

通过将 Redis 兼容性与针对固态介质优化的存储架构相结合，EloqKV 使应用能够管理显著更大的数据集，同时降低对昂贵且日益紧张的 DRAM 资源的依赖。

EloqKV 也可利用 DRAM 缓存极热数据，在保证超低延迟的同时，让大部分数据集驻留在高性能 SSD 上。

总而言之，通过用 NVMe SSD 替代昂贵的 DRAM 来承载大部分数据体量，EloqKV 打破了基于 DRAM 的系统的硬容量天花板，并为大规模工作负载带来最高约 **10 倍的总拥有成本（TCO）降低**。
