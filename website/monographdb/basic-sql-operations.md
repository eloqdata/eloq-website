...
# Explore SQL with MonographDB

After successfully deploying the MonographDB cluster, you can connect to the MonographDB to execute the corresponding SQL statement. MonographDB is compatible with MySQL, you can use the MySQL client to connect to MonographDB, and can use MySQL statements directly in most of cases. (**Note**: Some Mysql statements are not supported, see [Comparison with Mysql Compatibility](./mysql-compatibility.md)
...
The database in MonographDB is basically consistent with the definition in the MySQL context. Database (database) can be defined as a collection of tables, indexes and other related objects.
