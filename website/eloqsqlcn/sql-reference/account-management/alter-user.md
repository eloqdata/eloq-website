...
- `user_specification`：需要修改的 EloqSQL 账户名，账户名的指定方式与[`CREATE USER`](./create-user.md)语句相同。
- `authentication_option`:修改账户的身份验证方式。EloqSQL 支持同时对一个账户使用多种身份验证方式，例如密码或者 socket。具体可以查看[`CREATE USER`](./create-user.md)中的验证方式。
