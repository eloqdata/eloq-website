...
# EloqSQL 数据库集群部署、使用指南

本文档描述了在多台服务器上快速部署、使用 EloqSQL 集群的步骤。

## 在多机上安装部署数据库集群

- 适用场景：希望用多台服务器，体验 EloqSQL 的完整拓扑的集群。

本节介绍如何配置 EloqSQL 拓扑的 YAML 文件部署 EloqSQL 集群。

### 部署准备

部署数据库之前，请先完成[部署前配置](./basic-sql-operations.md)

联系我们获取 EloqSQL 安装包。其中，Eloq Waiter 是一个用于开发与管理 EloqSQL 的工具包，其中包含`cluster_mgr`, 一个用于集群安装部署和管理的命令行工具用于安装和管理 EloqSQL 集群。
...
对于 EloqSQL 集群拓扑，可以通过更改 YAML 文件按需配置所需要的集群数量，在本次测试中，集群拓扑如下表所示：
...
- 修改集群配置文件
  ...
  - `monitor`：配置 EloqSQL 的相关监控软件（prometheus、 grafana）的远程下载网址、安装位置以及安装端口。**目前 monitor 需要安装在 storage_service 所在节点**。
...
- 访问集群
  - 访问 EloqSQL 数据库，默认情况下，`mysql`安装在`/data/opt/mono-poc/eloq-tx-service-release/install/`下，进入到该目录下，使用 socket 方式来连接数据库。更多连接方式请参考[客户端连接](./connect-to-monodb/connect-by-client.md)。
    ```shell
...
推荐使用 [mydumper](https://github.com/eloqdb/mydumper) 导出/导入数据.

```shell
# for ubuntu20.04
wget https://www.eloqdata.com/download/mydumper/mydumper_focal.tar.gz
# for ubuntu22.04
wget https://www.eloqdata.com/download/mydumper/mydumper_jammy.tar.gz
...
- 如果你刚刚部署好一套 EloqSQL 本地测试集群：
  - 学习 [EloqSQL SQL 操作](./basic-sql-operations.md)
