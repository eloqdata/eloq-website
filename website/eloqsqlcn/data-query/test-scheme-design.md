---
title: Eloq测试数据库构建
summary: Eloq测试数据库、测试数据表创建
sidebar_position: 3
---

# 手动导入表结构和数据

我们提供了供用户测试的数据库初始化脚本，用户可以运行以下 SQL 语句
...
insert  into `orderTable`(`order_id`,`order_name`) values (1,'shkstart'),(2,'tomcat'),(3,'dubbo');
...
| 字段名 | 类型 | 含义 |
|--------------|---------------|---------------------------------------|
| country_id | char(2) | 国家的缩写 |
| country_name | varchar(40) | 国家的名称 |
| region_id | int(11) | 区域号标识(关联至[regions](#regions表)) |

## `regions`表

| 字段名      | 类型        | 含义           |
| ----------- | ----------- | -------------- |
| region_id   | int(11)     | 区域的唯一标识 |
| region_name | varchar(25) | 区域名称       |

## `departments`表

| 字段名          | 类型        | 含义                                            |
| --------------- | ----------- | ----------------------------------------------- |
| department_id   | int(4)      | 工作部门的唯一标识                              |
| department_name | varchar(30) | 工作部门名称                                    |
| manager_id      | int(6)      | 部门主管唯一标识                                |
| location_id     | int(4)      | 部门办公地点（关联至[locations](#locations表)） |
...
| 字段名         | 类型        | 含义                                               |
| -------------- | ----------- | -------------------------------------------------- |
| location_id    | int(4)      | 地址的唯一标识                                     |
| street_address | varchar(40) | 地址所在的街道名称                                 |
| postal_code    | varchar(12) | 地址所在区的邮政编码                               |
| city           | varchar(30) | 地址所在的城市名称                                 |
| state_province | varchar(25) | 地址所在的州府名称                                 |
| country_id     | char(2)     | 地址所在的国家标识(关联至[countries](#contries表)) |
