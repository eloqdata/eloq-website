...
bar1_read = ax1.bar(index_read - 1.5*bar_width, eloqkv_d8, bar_width, alpha=0.8, color=globals.group_3, label='EloqKV Core32 Disk8')
bar2_read = ax1.bar(index_read - 0.5*bar_width, eloqkv_d10, bar_width, alpha=0.8, color=globals.group_4, label='EloqKV Core32 Disk10')
bar3_read = ax1.bar(index_read + 0.5*bar_width, eloqkv_d8_48, bar_width, alpha=0.8, color=globals.group_5, label='EloqKV Core48 Disk8')
bar4_read = ax1.bar(index_read+ 1.5*bar_width, eloqkv_d10_48, bar_width, alpha=0.8, color=globals.group_6, label='EloqKV Core48 Disk10')
...
ax1.set_title('Scale CPU Write Workload Comparison', color='white', fontsize=globals.title_size)
...
ax2 = ax1.twinx()
line1_read = ax2.plot(index_read, eloqkv_d8_lat, '--', marker='o', color=globals.group_3, label='EloqKV 32C 1D Latency', linewidth=3)
line2_read = ax2.plot(index_read, eloqkv_d10_lat, '--', marker='o', color=globals.group_4, label='EloqKV 32C 2D Latency', linewidth=3)
line3_read = ax2.plot(index_read, eloqkv_d8_48_lat, '--', marker='o', color=globals.group_5, label='EloqKV 32C 4D Latency', linewidth=3)
line3_read = ax2.plot(index_read, eloqkv_d10_48_lat, '--', marker='o', color=globals.group_6, label='EloqKV 32C 6D Latency', linewidth=3)
...
plt.savefig('eloqkv_scale_cpu_set.png')  # Saves the plot as 'my_plot.png' in the current directory
plt.close()
