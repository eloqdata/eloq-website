...
bar1_read = ax1.bar(index_read - 2*bar_width, eloqkv_d1, bar_width, alpha=0.8, color=globals.group_2, label='EloqKV Core32 Disk1')
bar2_read = ax1.bar(index_read - 1*bar_width, eloqkv_d2, bar_width, alpha=0.8, color=globals.group_3, label='EloqKV Core32 Disk2')
bar3_read = ax1.bar(index_read, eloqkv_d4, bar_width, alpha=0.8, color=globals.group_4, label='EloqKV Core32 Disk4')
bar4_read = ax1.bar(index_read+ bar_width, eloqkv_d6, bar_width, alpha=0.8, color=globals.group_5, label='EloqKV Core32 Disk6')
bar5_read = ax1.bar(index_read + 2*bar_width, eloqkv_d8, bar_width, alpha=0.8, color=globals.group_6, label='EloqKV Core32 Disk8')
...
ax1.set_title('Scale Disk Write Workload Comparison', color='white', fontsize=globals.title_size)
...
ax2 = ax1.twinx()
line1_read = ax2.plot(index_read, eloqkv_d1_lat, '--', marker='o', color=globals.group_2, label='EloqKV 32C 1D Latency', linewidth=3)
line2_read = ax2.plot(index_read, eloqkv_d2_lat, '--', marker='o', color=globals.group_3, label='EloqKV 32C 2D Latency', linewidth=3)
line3_read = ax2.plot(index_read, eloqkv_d4_lat, '--', marker='o', color=globals.group_4, label='EloqKV 32C 4D Latency', linewidth=3)
line3_read = ax2.plot(index_read, eloqkv_d6_lat, '--', marker='o', color=globals.group_5, label='EloqKV 32C 6D Latency', linewidth=3)
line3_read = ax2.plot(index_read, eloqkv_d8_lat, '--', marker='o', color=globals.group_6, label='EloqKV 32C 8D Latency')
...
# Show plot
#plt.show()
plt.savefig('eloqkv_scale_disk_set.png')  # Saves the plot as 'my_plot.png' in the current directory
plt.close()
