...
Even when a database is fully ACID, EloqKV still contains several innovative technologies to reduce overhead. For example, latency of WAL logging highly depends on logging data throuput. In EloqKV, logging can scale independently of the writers, thus reducing the logging latency when more hardware is provided (we have to obey basic physics, afterall). We also avoid using the expensive 2-phase commit protocol for our distributed transactions. The details of this technology will be discussed in a blog or paper in the future.

## Scale as You See Fit

EloqKV is built to be scalable from ground up. Each resource types, including memory, CPU cores, logging SSD devices, and persistent storage can be scaled independently. Such full scalability is especially important in the cloud era, as we can easily buy each type of resources from public cloud providers. Your workload is latency sensitive and you need to keep all data in memory? Reserve a bunch of virtual machines with large memories. You have a lot of updates that need to be persisted? Just buy a few extra EBS volumes as logging devices. Your data size is humongous? Use cloud storage such as DynamoDB and S3 to store all your data.
