...
const EloqSQLDownloadPage: React.FC = () => {
  const triggerDownload = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const software_version = "0.4.14";
    const kvtype = "cassandra";

    const downloadUrl = `https://download.eloqdata.com/eloqsql/${kvtype}/eloqsql-${software_version}-${os_type}-${arch_type}.tar.gz`;

    const link = document.createElement("a");
    link.href = downloadUrl;
    link.setAttribute(
      "download",
      `eloqsql-${software_version}-${os_type}-${arch_type}.tar.gz`
    );
...
export default EloqSQLDownloadPage;
