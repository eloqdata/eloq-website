...
使用此`REVOKE`语句，用户必须具有全局`CREATE USER`权限或 mysql 数据库的`UPDATE`权限。
...
- `ADMIN OPTION`：表示对角色的管理权限。
  在 MonographDB 中支持的 priv_type 与 priv_level 对应如下
...
1. 取消 MonographDB 账户的特定权限
   以`'root'@'localhost'`用户登录到 MonographDB
...
> **注意**
> 上述语句执行成功的前提是角色 monorole1 的所有权用户是`'root'@localhost'`。
