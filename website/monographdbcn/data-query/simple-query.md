...
1.  构建 MonographDB 集群（可使用[单节点构建](../quick-start.md)或[多节点构建](../cluster-deployment.md)）
2.  [导入或创建相应的测试数据表](./test-scheme-design.md)
3.  [连接到 MonographDB](../guide-connect-to-monographdb.md)

## 基础查询

MonographDB 基于 MySQL 开发，所以支持 MySQL 提供的标准 SELECT 语句做查询，包括利用 WHERE,GROUP Y,ORDER BY,LIMIT 等进行数据的筛选、排序，限制查询结果数量等复杂命令。
...
通过观察查询结果你会发现，在使用 `LIMIT` 语句之后，查询的时间明显缩短，这是因为 MonographDB 对 `LIMIT` 子句做了相应的优化。
...
很多时候，我们需要的数据同时存放在多个表中，这个时候我们需要使用`JOIN`命令进行多表连接的操作使得两张或多张表的数据组合在一起，展现出来。Monograph 支持基本的内连接`INNER JOIN`，左外连接`LEFT OUTER JOIN`以及右外连接`RIGHT OUTER JOIN`
