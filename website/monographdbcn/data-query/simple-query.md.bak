...
## 准备工作
在阅读本章之前，你需要做以下准备工作：
 1. 构建MonographDB集群（可使用[单节点构建](../quick-start.md)或[多节点构建](../cluster-deployment.md)）
 2. [导入或创建相应的测试数据表](./test-scheme-design.md)
 3. [连接到MonographDB](../guide-connect-to-monographdb.md)
## 基础查询
MonographDB基于MySQL开发，所以支持MySQL提供的标准SELECT语句做查询，包括利用WHERE,GROUP Y,ORDER BY,LIMIT等进行数据的筛选、排序，限制查询结果数量等复杂命令。
通过我们提供的数据库创建示例，可以创建`mono`数据库以及相应的数据表，其中创建的数据表`employees`存放了公司员工的一些基本信息，可以通过 `SELECT ... FROM ...` 查看相应数据表的所有数据信息。
通过MySQL Client或者Mysql shell中终端连接到数据库后，输入相应的命令
```sql
...
```
通过观察查询结果你会发现，在使用 `LIMIT` 语句之后，查询的时间明显缩短，这是因为MonographDB 对 `LIMIT` 子句做了相应的优化。
## 分组聚合查询
...
## 多表连接
很多时候，我们需要的数据同时存放在多个表中，这个时候我们需要使用`JOIN`命令进行多表连接的操作使得两张或多张表的数据组合在一起，展现出来。Monograph支持基本的内连接`INNER JOIN`，左外连接`LEFT OUTER JOIN`以及右外连接`RIGHT OUTER JOIN`

例如，想要知道每个员工的所在具体部门信息，而不仅仅是部门编号，需要将员工表`employees`与部门表`departments`进行连接，在下面的SQL语句中，通过`JOIN`声明要将左表`employees`和右表`departments`的数据行以内连接的方式进行连接，连接条件为`employees.department_id=departments.department_id`。
