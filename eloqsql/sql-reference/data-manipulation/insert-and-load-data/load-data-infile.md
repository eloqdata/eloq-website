---
title: LOAD DATA INFILE | EloqSQL SQL Statement Reference
summary: An overview of the usage of LOAD DATA INFILE for the EloqSQL database.
---

# LOAD DATA INFILE

The `LOAD DATA INFILE` can be used to load data into a EloqSQL table.

> **Note**
> When using the `LOAD DATA INFILE` statement, the table must already exist in the database. EloqSQL will return an error if you try to load data into a table that does not exist.

## Examples

Load local files into a EloqSQL database using `LOAD DATA LOCAL INFILE`
