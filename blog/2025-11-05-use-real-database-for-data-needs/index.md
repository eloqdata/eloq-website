...
Distributed systems are already hard to reason about. Add multiple data tiers like Redis, Kafka, and a transactional database and you multiply the number of durability and consistency scenarios. Each component has its own guarantees: Kafka offers a choice of *"at-least-once"*, *"at-most-once"*, *"exact-once"* delivery, and whether you need flush every writes to achieve durability is sometimes [debated](https://jack-vanlightly.com/blog/2023/4/24/why-apache-kafka-doesnt-need-fsync-to-be-safe). Redis is *eventually consistent*, meaning data can be stale, and provide *replication, write-ahead-log and snapshot* as [persistency mechanisms](https://redis.io/technology/durable-redis/). Your SQL database is usually transactionally consistent with full ACID guarantee, but only within its own boundary. Stitch them together, and you're left trying to reason about correctness in a house of cards.
...
You can retrofit Postgres with external layers: [Citus](https://www.citusdata.com/) for sharding, [Neon](https://neon.com/) for decoupled storage, [TimeScale (now TigerData)](https://www.tigerdata.com/) and [PGVector](https://github.com/pgvector/pgvector) for handling time-series and vector data. But this is often not optimal, and often just recreates the complexity we were trying to eliminate (i.e. they do not compose). When serious modification is needed, such as making it fully multi-writer capable as in [CockroachDB](https://www.cockroachlabs.com/) and [YugabyteDB](https://www.yugabyte.com/) or streaming capable as in [Materialize](https://materialize.com/) and [RisingWave](https://risingwave.com/), a major rewrite is often needed and often only the wire-protocol can be reused. Extensions like logical replication, foreign data wrappers, and background workers are impressive engineering feats, but they're patches on a monolith, not blueprints for a unified data platform.
